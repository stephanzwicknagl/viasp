...
class ProgramAnalyzer(DependencyCollector, FilteredTransformer):
    ...
    def __init__(self):
        ...
        self.positive_conditions: Dict[Tuple[str, int],
                                        Set[Rule]] = defaultdict(set) # type: ignore
    ...
    def register_symbolic_dependencies(self,
                                    deps: Dict[Literal, List[Literal]]):
    ...
    def add_program(self,
                    program: str,
                    registered_transformer: Transformer = None) -> None:
    ...
    def visit_ShowTerm(self, showTerm: ast.ShowTerm): # type: ignore
        ...
        new_body_literals.insert(
            ...
            ast.Literal(
                loc,
                ast.Sign.NoSign,
                ast.Function(loc, self.conflict_free_showTerm, [showTerm.term], 0)))
        # Remove duplicates but preserve order
        new_body_literals = [
            x for i, x in enumerate(new_body_literals) if x not in new_body_literals[:i]
        ]
        # rename variables inside body aggregates
