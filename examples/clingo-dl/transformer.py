from clingo.ast import Transformer, AST
...
class Transformer(Transformer):
    ...
    def make_dl_literals(self, location: AST, term: AST) -> Tuple[Union[str, AST], AST]:
        if getattr(term, "ast_type", List) == ast.ASTType.SymbolicTerm and \
            ...
            inner_var = ast.Function(location, '', term, 0) if isinstance(term, list) else term
            inner_X = ast.Variable(location, conflict_free_x)
            outer_function = ast.Function(location, 'dl', [inner_var, inner_X], 0)
    ...
    def visit_TheoryAtom(self, atom: AST, **kwargs: Any) -> Tuple[AST, AST, List[str]]:
        ...
        if term.name == "diff":
            ...
            self.visit_children(atom, theory_content=content,  collect_guard=guard, collect_operations=sequence_operations, **kwargs)
            for i in content:
    ...
    def visit_Rule(self, rule: AST, **kwargs: Any) -> Union[AST, List[AST]]:
        ...
        if rule.head.ast_type == ast.ASTType.TheoryAtom:
            ...
            for conditional_literal in conditionals:
                ...
                aggregate = ast.Aggregate(loc, guard, [conditional_literal], None)
                ...
                condition = [ast.Literal(location,
                    ...
                    [ast.Guard(5,
                            ast.Interval(location, 
                                ...
                                ast.SymbolicTerm(location, 
                                    self.max)))]))]
                conditional_literals.append(ast.ConditionalLiteral(location, dl_lit, condition))
